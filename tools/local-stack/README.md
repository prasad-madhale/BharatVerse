...
The stand-in's own logic (the SQL splitter and the gateway's tokens and passwords) has tests that need no database:
`python -m pytest tools/local-stack/tests`. CI does not run them.
