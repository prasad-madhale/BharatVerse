...
- `backend/services/search_service.py` (`SearchService.search_articles`) + `backend/api/search.py` (`GET /api/v1/articles/search?q=...&limit=...`, the path `design.md` specifies), wired into `main.py`. Searches `title`/`summary` (still not full article content, which lives in Storage) through the `search_articles` SQL function in `schema.sql` (called with `rpc`), which ranks by relevance: a title term is weighted A, ten times a summary term, so a title match beats a few passing mentions, and date, `created_at` and id break ties. **The function is part of the manual migration** along with the `search_vector` column: search fails until it exists. Verified against the local Postgres with articles crafted so date order and relevance order disagree.
