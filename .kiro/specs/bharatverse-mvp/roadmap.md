...
- **backend/**: `config.py`, the Supabase client wrapper, `database/schema.sql`, `models/article.py` (`ArticleRecord`), `services/article_service.py` (`list_recent_titles` for topic dedup, `list_recent_articles` for the home screen, `load_article` now public so `SearchService` can reuse it), `api/articles.py` (`GET /articles` list + `/daily` + `/{id}`), and `main.py` are all real and verified live end-to-end. `services/search_service.py` + `api/search.py` (`GET /search?q=...`) are new this pass -- code-complete and unit-tested, but **not yet verified live** (see Phase 2 below for why). Likes still doesn't exist.
...
- `backend/services/search_service.py` (`SearchService.search_articles`) + `backend/api/search.py` (`GET /api/v1/search?q=...&limit=...`), wired into `main.py`. Searches `title`/`summary` (still not full article content, which lives in Storage) via Postgres FTS, ordered by publication date rather than text-match rank -- rank ordering would need a Postgres RPC function, left out of this pass.
