...
- `backend/main.py` — FastAPI entry point, wires config, CORS, `/health`, and the articles router. Verified live: booted the server, confirmed `/health` returns 200 and `/api/v1/articles/daily` correctly reaches the Supabase client before failing (dead project -- see Risks below).
...
- **Blocked on**: the dev Supabase project is permanently deactivated (free-tier 90-day pause limit, not reversible -- see Risks below). Needs a **fresh** Supabase project: apply `schema.sql` (including the updated `users` table), create the `articles` Storage bucket, point `.env` at the new project, then re-run `scrapper_main.py` output through `ArticleService.save_article` and hit the live API to close the loop on Phase 0's exit criteria.
...
- **Resolved (bad news, then a clear path)**: the dev Supabase project is not just stuck-paused — its status is `INACTIVE` (not `PAUSED`), and the project was created ~140 days ago. Supabase permanently deactivates free-tier projects paused past 90 days; per their own docs this is **not reversible** (no resume, no CLI trick, no support ticket brings it back). Support ticket SU-411542 is now moot. Since Phase 0 was blocked on this project from the start, `schema.sql` was never confirmed applied and no article was ever actually saved to it — so there's no real data lost, just infrastructure to recreate. **Next step**: create a fresh Supabase project, apply `schema.sql`, create the `articles` Storage bucket, point `.env` at the new project (update `SUPABASE_URL`/`SUPABASE_ANON_KEY`/`SUPABASE_SERVICE_ROLE_KEY` directly in `.env` — never paste these in chat), then re-run `scrapper_main.py` → `ArticleService.save_article` → hit the live API to finally close the Phase 0 loop.
...
### Exit criteria
One documented sequence takes a hardcoded topic through scrapper → Supabase → backend API → Flutter screen, showing real LLM-generated content on a simulator/device. **Currently blocked on both remaining links** (need a fresh Supabase project, the old one is permanently dead; no working simulator/browser-screenshot path in this environment) — see Risks above.
