"""SaveService tests. RLS is bypassed on this path, so each query's user_id filter is asserted on the wire."""
...
class TestSaveArticle:
    ...
    async def test_unknown_article_raises_article_not_found(self, mock_get_supabase):
        error = {
            "code": "23503",
            "message": 'insert or update on table "saved_articles" violates foreign key constraint '
            '"saved_articles_article_id_fkey"',
        ...
        with pytest.raises(ArticleNotFoundError):
            await SaveService().save_article("user-1", "art_missing")
    ...
    async def test_foreign_key_violation_on_the_user_is_not_reported_as_missing_article(self, mock_get_supabase):
        ...
        with pytest.raises(APIError):
            await SaveService().save_article("user-x", "art_1")
...
class TestUnsaveArticle:
    ...
    async def test_deletes_only_this_users_row_for_this_article(self, mock_get_supabase):
        ...
        await SaveService().unsave_article("user-1", "art_1")
...
class TestIsSaved:
    ...
    async def test_true_when_row_present_and_query_is_scoped_to_the_user(self, mock_get_supabase):
        ...
        assert await SaveService().is_saved("user-1", "art_1") is True
    ...
    async def test_false_when_no_row(self, mock_get_supabase):
        ...
        assert await SaveService().is_saved("user-1", "art_1") is False


class TestGetUserSaves:
    @pytest.mark.asyncio
    @patch("backend.services.save_service.get_supabase")
    async def test_returns_full_articles_most_recent_first_scoped_to_the_user(self, mock_get_supabase):
        ...
        wire = use_admin_wire(mock_get_supabase, rows(*saved), blob=article_blob())

        articles = await SaveService().get_user_saves("user-1")
    ...
    async def test_passes_custom_limit(self, mock_get_supabase):
        ...
        await SaveService().get_user_saves("user-1", limit=5)

        assert wire.requests[0].url.params["limit"] == "5"

    @pytest.mark.asyncio
    @patch("backend.services.save_service.get_supabase")
    async def test_empty_when_no_saves(self, mock_get_supabase):
        use_admin_wire(mock_get_supabase, rows())

        assert await SaveService().get_user_saves("user-1") == []


class TestGetArticleSaveCount:
    @pytest.mark.asyncio
    @patch("backend.services.save_service.get_supabase")
    async def test_reads_the_total_from_a_count_only_request(self, mock_get_supabase):
        ...
        assert await SaveService().get_article_save_count("art_1") == 7
    ...
    async def test_zero_when_no_count_is_returned(self, mock_get_supabase):
        ...
        assert await SaveService().get_article_save_count("art_1") == 0
