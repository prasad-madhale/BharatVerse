...
def article_strategy(draw):
    ...
    article_id = f"art_{draw(st.integers(min_value=20200101, max_value=20301231))}_{draw(st.integers(min_value=1, max_value=999)):03d}"
    ...
    return {
        ...
        'date': draw(st.dates(min_value=date(2020, 1, 1), max_value=date(2030, 12, 31))).isoformat(),
        'reading_time_minutes': draw(st.integers(min_value=5, max_value=30)),
        'author': draw(st.sampled_from(['BharatVerse AI', 'Historical AI', 'Content Generator'])),
        'tags': draw(st.lists(safe_text_tag, min_size=1, max_size=5)),
        'content_file_path': f"articles/{draw(st.dates(min_value=date(2020, 1, 1), max_value=date(2030, 12, 31))).isoformat()}/{article_id}.json"
    }
